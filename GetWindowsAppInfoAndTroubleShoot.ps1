    ...
    while (($UserPrompt -ne "q") -or ($UserPrompt -ne "Q")) {
        ...
        switch ( $UserPrompt ) 
        {
            ...
            "5" {
                ...
                CheckIfAppExistAtProvision $UserPrompt
                
                #$ProvisionApp = CheckIfAppExistAtProvision $UserPrompt $
                #
                #if ($ProvisionApp -ne "NONE") {
                #    Write-Host "     Found! The app below matched <$($UserPrompt)> and is staged at the provision (OS) level:" -ForegroundColor Green
                #    Write-Host "     $($ProvisionApp)`n" -ForegroundColor Yellow
                #} else {
                #    Write-Host "     Not Found! There are currently no match for <$($UserPrompt)> staged at the provision (OS) level.`n" -ForegroundColor Red                   
                #}
